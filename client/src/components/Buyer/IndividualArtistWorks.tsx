  ...
  useEffect(() => {
    const fetchArtworks = async () => {
      try {
        ...
        const allArtworks = res.data.data || [];

        // ✅ Filter artworks to show only those with status "available"
        const availableArtworks = allArtworks.filter(
          (art) => art.status === "available"
        );

        setArtworks(availableArtworks);
        setArtistName(availableArtworks[0]?.artistId?.firstname || "Artist");
        setArtistImage(
          availableArtworks[0]?.artistId?.image?.filename || "default.png"
        );
      } catch (err) {
